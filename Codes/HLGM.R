...
Objfunc = function(X, Z, B, Omega, lambda, gamma,
  ...
  Obj = Obj +
    ...
    sum(pi.s*(sum(Z^2) - 2*sum(Z)*mu.s + q*mu.s^2 + phi.s.sq)/sig.s^2)/2
  
  Obj
}

## Evaluate outputs from a model object
eval.Omega = function(Omega, B, Omega0){
  # adjacency matrix
  Adj.matrix = Omega
  ...
  c((sum(Omega!=0 & Omega0!=0)-p)/(sum(Omega0!=0)-p),
...
## Main algorithm
hlgm = function(X, B_init=NULL, Theta_init=NULL, init.option=1,
...
hlgm1 = function(X, B_init=NULL, Theta_init=NULL, init.option=1,
                lambda=NULL, gamma=NULL, K=3, eta=1e-4,
                refit.B=TRUE, tol=1e-3, grad.max=1e3,
                maxit=20, eps=1e-6, max.epoch=1e2,VERBOSE=TRUE){
